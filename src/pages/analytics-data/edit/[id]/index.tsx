...
import { getAnalyticsDataById, updateAnalyticsDataById } from 'apiSdk/analytics-data';
...
function AnalyticsDataEditPage() {
  ...
  const handleSubmit = async (values: AnalyticsDataInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateAnalyticsDataById(id, values);
      mutate(updated);
      resetForm();
